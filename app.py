...
def load_model():
    ...
    try:
        ...
        if isinstance(loaded, tuple):
            model, best_threshold = loaded
            print(f"✅ Model and threshold loaded (threshold = {best_threshold:.4f})")
        elif hasattr(loaded, 'predict_proba'):
            model = loaded
            print("✅ Model loaded (using default threshold = 0.5)")
        else:
            raise ValueError("Invalid model format")
            
        # Test prediction with dummy data
        test_input = pd.DataFrame([[0]*len(FEATURE_ORDER)], columns=FEATURE_ORDER)
...
def api_predict():
    try:
        data = {}
        for f in fields:
            raw = request.form.get(f["name"], "0")  # Default to "0" if missing
        ...
        # Enhanced output with risk levels
        if proba >= 0.75:
            risk = "High Risk"
            recommendation = "Consult a doctor immediately"
        elif proba >= best_threshold:
            risk = "Moderate Risk"
            recommendation = "Recommended to see a doctor"
        elif proba >= 0.3:
            risk = "Low Risk"
